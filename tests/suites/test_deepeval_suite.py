...
def _cases(report: dict) -> list[dict]:
    cases: list[dict] = []
    families = report.get("data", {}).get("families", {})
    for family_key, family_report in sorted(families.items()):
        ...
        for index, sample in enumerate(samples, start=1):
            ...
            score = float(sample.get("comparison_score", 0.0))
            passed = bool(sample.get("matched_expected", False))
            cases.append(
                {
                    ...
                    "details": {
                        ...
                        "evaluation_engine": family_report.get("evaluation_engine"),
                    },
                }
            )
    return cases
...
def test_deepeval_case(case):
    attach_case(
        parent_suite="Agentic Evaluation",
        suite="DeepEval Agent",
